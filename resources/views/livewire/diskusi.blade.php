            ...
            @foreach ($komentars as $komentar)
                <li class="clearfix">
                    <div class="message-data text-{{ $komentar->user_id == Auth::id() ? 'right' : 'left' }}">
                        {{ $komentar->user->name }}
                        <img src="{{ $komentar->user->gambar ?  asset('img-upload/' . $komentar->user->gambar) : 'https://mdbcdn.b-cdn.net/img/Photos/new-templates/bootstrap-chat/ava6-bg.webp' }}" alt="avatar">
                    </div>
